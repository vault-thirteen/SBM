...
plain ASCII Text Symbols and Pixel Array is encoded using the binary Format.
...
When the Object of the SBM Model Type is stored into the Stream, it stores
only the Bytes Array and omits the internal Array of Bit Objects. The same
happens when the SBM Model Type is read from the Stream -- the Bytes are
received from the Stream, not the Bits.
